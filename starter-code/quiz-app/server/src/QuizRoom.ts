...
export class QuizRoom {
  ...
  handleAnswer(playerId: string, choiceIndex: number): void {
    ...
    if (this.answers.has(playerId)) return // double answer -> ignore
    ...
    // Si tous les joueurs ont repondu -> terminer immediatement
  ...
  private broadcastQuestion(): void {
    const question = this.questions[this.currentQuestionIndex]
    const { correctIndex: _correctIndex, ...questionWithoutAnswer } = question
    this.broadcastToAll({
      type: 'question',
  ...
  private broadcastResults(): void {
    ...
    const distribution = new Array(question.choices.length).fill(0) as number[]
    ...
    for (const [playerId, player] of this.players.entries()) {
      scoresObj[player.name] = this.scores.get(playerId) ?? 0
    }
